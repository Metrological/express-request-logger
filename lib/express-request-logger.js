...
var configure = function(app, options) {
    ...
    var redisGetter = function() {
        if (!redisClient) {
            ...
            redisClient = redis.createClient(redisConfig[0], redisConfig[1], {connect_timeout: 10000});

            redisClient.on("error", function (err) {
                if (err.message && (err.message.indexOf("Redis connection to") >= 0) && (err.message.indexOf("failed - connect ECONNREFUSED") >= 0)) {
